...
async function fetchPictureList() {
	try {
		const response = await fetch('https://reed-family-backend-b01b489ec3fe.herokuapp.com/api/pictures');
		if (!response.ok) {
...
async function fetchRecipeList() {
	try {
		const response = await fetch('https://reed-family-backend-b01b489ec3fe.herokuapp.com/api/recipes');
		if (!response.ok) {
			throw new Error(`HTTP error! Status: ${response.status}`);
		}
		const data = await response.json();
		...
		<Router>
			<div className="App">
				...
				<Routes>
					...
					<Route path="/pictures" element={<Pictures pictureList={pictureList}/>} />
					<Route path="/" element={<Home pictureList={pictureList} recipeList={recipeList}/>} />
				</Routes>
			</div>
		</Router>
	);
